...
<style type="text/css">
	...
	.bar-incident{
		background-color: #C347ED;/*#F142F4;*/
	}
	#workers-div {
		overflow: auto;
		max-height: 500px;
...
*/
	...
	#process-table tr td{vertical-align: middle;padding: 4px 8px; min-width: 70px;}
	#process-table tr td:nth-child(1){min-width:200px; overflow: hidden; text-wrap: none}
	#process-table tr td:nth-child(3){text-align: center; width: 70px;}
	#process-table tr td:nth-child(4){text-align: center; width: 70px;}
	#process-table tr td:nth-child(5){text-align: center; width: 70px;}
	.progress{margin: 0px;}
	.w-down{
		color: #bbb;
	}
				...
				<div class="col-md-6">
					<form id="bpmn-form" action="/${base}/rest/deployments/deployProcessDefinition" 
						method="post" enctype="multipart/form-data" class="form">
										...
										<input class="btn btn-default" style="clear:both" type="file" accept=".bpmn" name="file" id="file-input" onchange="form.submit()" />
					...
					<tbody>
						<#list procDefs as pd>
							<tr class="<#if pd.suspended>disabled</#if>">
								<td><a style="cursor: pointer;" onClick='parent.location="/camunda/app/cockpit/default/#/process-definition/${pd.id}/runtime"' />${pd.name!""}</a>
								<a data-proc-key="${pd.key}" onClick="handleDeleteProcDef(this.getAttribute('data-proc-key'))"><span style="cursor:pointer;float:right;color:#d9534f;padding-left:7" id="delete-${pd.key}" class="glyphicon glyphicon-remove-sign"></a>
								<a href="/${base}/modeler?procDefKey=${pd.key}" target="_blank"></span><span style="float:right;" id="edit-${pd.key}" class="glyphicon glyphicon-pencil"></span></a>
								</td>
								<td>${pd.key!"ERROR"}</td>
								<td>${pd.version!"ERROR"}</td>
								<td><button id="pv-${pd.key}" class="btn btn-default worker-view-btn" data-proc-key="${pd.key}">view</button></td>
								<td>
									<#if pd.suspended>suspended<#else>active</#if>
									<img id="spinner_${pd.key}" src="/${base}/images/spinner.20.gif" style="display:none;" />
								</td>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<br/>
				<p>Warning: All data for this process definition will be removed (run history, logs, pending runs, etc...)</p>
...
<div class="modal fade" id="delete-error-modal" role="dialog" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<p>There was an error deleting process definition '<span id="fail-delete-proc-name"></span>'.</p>
				<br/>
...
<div class="modal fade" id="ajax-error-modal" role="dialog" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-body">
				<p>There was an error loading the status of processes. Please make sure that CWS is up and running.</p>
			</div>
