...
REPLY2="X"
while [[ ! $REPLY2 =~ ^(y|Y|n|N)$ ]]; do
	read -p "The directory at ${ROOT}/fleets/${TAG} will be permanently deleted (if exists).  Proceed (Y/N): " REPLY2
	if [[ $REPLY2 =~ ^(n|N)$ ]]
	then
		exit 1;  # abort installation process
	fi
...
echo "Removing dir ${ROOT}/fleets/${TAG} ..."

rm -rf ${ROOT}/fleets/${TAG}
mkdir -p ${ROOT}/fleets/${TAG}
cd ${ROOT}/fleets/${TAG}

cp ${ROOT}/add_workers.tf .
#cp ${ROOT}/terraform.tfvars .
cp ${ROOT}/dest_temp ./destroy_cloud_fleet.sh

echo "tag = \"$TAG\""                  >> ./terraform.tfvars
echo "numWorkers = \"${NUM_WORKERS}\"" >> ./terraform.tfvars

#
# RUN TERRAFORM APPLY
#
touch console_hostname.txt
touch db_hostname.txt
touch worker_hostnames.txt
terraform init
...
echo
echo "+---------------------------------------------------------------------------------------------------"
echo "| CLOUD FLEET LAUNCHED! "
echo "+-----------------------"
echo "|   Working directory : `pwd`"
echo "|   Console hostname  : `cat console_hostname.txt`"
...
echo "|"
echo "|   NOTE: Once you are done using your fleet, destroy it by running: "
echo "|         `pwd`/destroy_cloud_fleet.sh                               "
echo "+---------------------------------------------------------------------------------------------------"
echo
