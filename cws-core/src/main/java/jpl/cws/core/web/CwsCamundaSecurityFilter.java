...
/**
 * Security filter specific to the CAMUNDA authentication scheme
 *
 * @author ghollins, jwood
 *
 */
public class CwsCamundaSecurityFilter extends CwsSecurityFilter {
	private static final Logger log = LoggerFactory.getLogger(CwsCamundaSecurityFilter.class);

	private String prop_cwsAdminUsername;
	private String prop_cwsAdminFirstname;
	private String prop_cwsAdminLastname;
	private String prop_cwsAdminEmail;

	private boolean createdInitialUser = false;

	public void init(FilterConfig filterConfig) {

		prop_cwsAdminUsername = filterConfig.getInitParameter("adminUserId");
		prop_cwsAdminFirstname = filterConfig.getInitParameter("adminFirstname");
		prop_cwsAdminLastname = filterConfig.getInitParameter("adminLastname");
		prop_cwsAdminEmail = filterConfig.getInitParameter("adminEmail");
	...
	public void doFilter(
		...
		if (contextPath.equals("/cws-ui")) {
			if (!createdInitialUser) {
				createInitialUser(prop_cwsAdminUsername, "changeme", prop_cwsAdminFirstname, prop_cwsAdminLastname, prop_cwsAdminEmail);
			}
		}
	...
	public synchronized void createInitialUser(String userId, String password, String firstName, String lastName, String email) {

		IdentityService identityService = (IdentityService)SpringApplicationContext.getBean("identityService");

		// Check if this is the installation setup, which will have user count equal to 0.
		// If so, then add the new admin user.
		if (identityService.createUserQuery().count() == 0) {

			if (identityService.createGroupQuery().groupId(Groups.CAMUNDA_ADMIN).count() == 0) {

				log.info("Adding Camunda Admin group...");

				org.camunda.bpm.engine.identity.Group group = identityService.newGroup(Groups.CAMUNDA_ADMIN);

				group.setName("camunda BPM Adminstrators");
				group.setType(Groups.GROUP_TYPE_SYSTEM);
				identityService.saveGroup(group);
			}
			else {

				log.info("No need to add Camunda Admin group since it already exists.");
			}
			...
			AuthorizationService authorizationService = (AuthorizationService)SpringApplicationContext.getBean("authorizationService");

			for (Resource res : org.camunda.bpm.engine.authorization.Resources.values()) {

				// Only add authorization if it hasn't been added.
				if (authorizationService.createAuthorizationQuery().resourceType(res).count() == 0) {
					...
					auth.addPermission(org.camunda.bpm.engine.authorization.Permissions.ALL);

					authorizationService.saveAuthorization(auth);
				}
			}

			identityService.createMembership(user.getId(), Groups.CAMUNDA_ADMIN);
		}
	...
	private boolean postCamundaLoginIfNecessary(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		// isHuman means the user is using web apps as a User
		//
		boolean isHuman = true;
		if (contextPath.equals("/cws-engine")) {
			isHuman = false; // cws-engine calls are always automated
		}
		if (isHuman) {
			try {
